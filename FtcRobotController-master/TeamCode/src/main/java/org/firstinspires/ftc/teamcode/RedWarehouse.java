...
@Autonomous(name = "Red Warehouse Full")
...
public class RedWarehouse extends LinearOpMode {
        ...
        if (opModeIsActive()) {
          ...
          /*
            ...
            if (objectDetected > objectAbsent) {
                ...
                objectDetected = 0; //initalized again
                objectAbsent = 0; //initalized again
                // The object is not on the right-most spot, so we move left to the middle spot
               Trajectory strafeMiddle = enigma.trajectoryBuilder(new Pose2d(0, 0, 0))
                        .lineToLinearHeading(new Pose2d(12, 12, Math.toRadians(0)))
                        .build();
                enigma.followTrajectory(strafeMiddle);
                sleep(1000);
    ...
    public void gyroCorrect()
    {
        ...
        for(int f = 1; f < 20; f++)
        {
            enigma.leftFront.setPower(-0.05 * (enigma.absHeading - enigma.originalHeading));
            enigma.leftRear.setPower(-0.05 * (enigma.absHeading - enigma.originalHeading));
            enigma.rightFront.setPower(0.05 * (enigma.absHeading - enigma.originalHeading));
            enigma.rightRear.setPower(0.05 * (enigma.absHeading - enigma.originalHeading));
            enigma.angles = enigma.imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYX, AngleUnit.DEGREES);
            enigma.absHeading = enigma.angles.firstAngle;
            telemetry.addData("Heading", enigma.absHeading);
            telemetry.update();
